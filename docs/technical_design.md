...
├── source_context.py    # Source-context cleaning, filtering, friendly source names, and card generation
├── incident_engine.py   # Practical incident-response question detection
├── incident_reports.py  # SOC incident report generation and incident log templates
...
* source-context display and formatting
...
* source-context display flow using `app/source_context.py`
...
SOC incident report and incident log generation has been moved into:

```text
app/incident_reports.py
```

Source-context cleaning, filtering, and card generation has been moved into:
...
incident_reports.py handles testable SOC report generation.
source_context.py handles testable source-context display helpers.
```
...
`source_context.py` contains source-context display helper logic.

It handles:

* source excerpt cleaning
* source excerpt language filtering
* low-value section filtering
* checklist-section filtering
* source-context card selection
* short and expanded source excerpt formatting
* friendly source area names
* friendly source type names
* HTML generation for source-context cards

This module was extracted from `app/main.py` so the Streamlit entry point can focus more on UI flow and so source-context behavior can be tested separately.
...
`incident_reports.py` generates SOC-oriented incident report content for practical incident-response questions.
...
* plain-text cleanup for downloaded report content
* short source notes for report handover use

This keeps incident report generation separate from the Streamlit UI in `app/main.py`.

That separation makes the SOC report feature easier to test with `pytest` without launching the full Streamlit interface, which is a small mercy in a universe already full of needless suffering.
...
* generating incident-response report text through `incident_reports.py`
* building relevant source-context cards through `source_context.py`
...
* practical explanations
* assessment checklists
* incident log templates generated by `incident_reports.py`
* SOC Markdown report download button
* relevant source context generated through `source_context.py`
...
## Source Context Display Design

CyberLex Sweden displays relevant source-context cards below supported answers.

The source-context display logic is handled in:

```text
app/source_context.py
```

This module cleans and formats source excerpts before they are shown in the Streamlit UI. It removes low-value authoring sections, hides example-question noise, filters obvious wrong-language source snippets, avoids duplicating checklist sections where better context exists, and creates short previews with expandable full text where needed.

The module also converts internal Markdown filenames into user-friendly source area names, such as showing `cyber_incident_response_playbook.md` as a readable incident-response source area instead of exposing a local file name to normal users.

Automated tests for this behavior are stored in:
...
* source-context excerpt cleaning and display helper behavior
...
The report generation logic is located in:

```text
app/incident_reports.py
```

The Streamlit app calls this module when `routing.py` and `incident_engine.py` identify a practical incident-response question.

Non-incident questions return an empty report, so normal legal or compliance answers do not show incident-report export behavior.
...
These tests check report sections, Swedish incident-log labels, non-incident report suppression, and suspicious-link SOC triage content.
...
* automated tests currently cover selected core logic, SOC report generation, source-context helper behavior, and routing behavior, but not the full UI
...
* more automated tests for source-context card selection and language filtering
...
* SOC Markdown report export through `app/incident_reports.py`
* source context module in `app/source_context.py`
* relevant source context display through `app/source_context.py`
* other matching source sections
...
* current automated test result: `50 passed`
...
The next major technical step is broader test coverage for source routing, case behavior, Auto language behavior, source-watch behavior, source-context edge cases, true vector search, and later a RAG-based answer mode that remains mandatory-source-grounded.
