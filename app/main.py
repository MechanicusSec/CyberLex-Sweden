...
def build_soc_report_download_filename(report_text):
    """
    Build a safe download filename for the SOC report.

    Expected examples:
    - cyberlex_soc_report_inc-2026-0042_20260624_1530.md
    - cyberlex_soc_report_20260624_1530.md
    - cyberlex_soc_incident_report.md
    """
    import re
    ...
    report_id_match = re.search(
        r"Report ID:\s*(CYBERLEX-SOC-(\d{8})-(\d{4}))",
        text,
        flags=re.IGNORECASE,
    )
    ...
    if not report_id_match:
        report_id_match = re.search(
            r"Rapport-ID:\s*(CYBERLEX-SOC-(\d{8})-(\d{4}))",
            text,
            flags=re.IGNORECASE,
        )

    incident_id_match = re.search(
        r"Incident ID / Ticket ID:\s*([^\n\r]+)",
        text,
        flags=re.IGNORECASE,
    )
    ...
    if not incident_id_match:
        incident_id_match = re.search(
            r"Incident-ID / Ärende-ID:\s*([^\n\r]+)",
            text,
            flags=re.IGNORECASE,
        )

    incident_id_filename_part = ""

    if incident_id_match:
        incident_id_text = incident_id_match.group(1).strip()

        if incident_id_text:
            incident_id_safe = incident_id_text.lower()
            incident_id_safe = re.sub(r"[^a-z0-9_-]+", "-", incident_id_safe)
            incident_id_safe = re.sub(r"-+", "-", incident_id_safe)
            incident_id_safe = incident_id_safe.strip("-_")

            if incident_id_safe:
                incident_id_filename_part = f"{incident_id_safe}_"

    if report_id_match:
        report_date = report_id_match.group(2)
        report_time = report_id_match.group(3)

        return f"cyberlex_soc_report_{incident_id_filename_part}{report_date}_{report_time}.md"
